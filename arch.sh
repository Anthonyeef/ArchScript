...
########################################
# 待確認項目：
# - CPU 頻率動態調整(https://wiki.archlinux.org/index.php/CPU_frequency_scaling)
########################################
...
step2(){
    ...
    adobe-source-han-sans-tw-fonts ttf-arphic-uming ttf-freefont \
		\
		alsa-utils at btrfs-progs cronie gparted linux-headers lsof ntfs-3g ntp openssh parted \
		...
		xorg-server-devel \
		\
		gnome gnome-extra gnome-system-monitor lightdm lightdm-gtk-greeter lilyterm numlockx \
		networkmanager networkmanager-openconnect networkmanager-openvpn networkmanager-pptp \
		networkmanager-vpnc \
		\
		banshee eom gst-libav gst-plugins-ugly mpv smplayer vlc \
		\
		fcitx fcitx-chewing fcitx-configtool fcitx-fbterm fcitx-gtk2 fcitx-gtk3 fcitx-qt4 \
		fcitx-qt5 libchewing npm \
		\
		gedit vim \
		\
	  firefox firefox-i18n-zh-tw
	...
	echo 'Configure IME ...'
	sed -i 's/^#\(zh_TW.UTF-8 UTF-8\)$/\1/g' /etc/locale.gen
	locale-gen
	...
	# 設定字型
	# 設定預設介面風格為 Ciliora-Prima (Ciliora-*系列基本都推薦) 須下載或預打包
	#mkdir /etc/skel/.themes
	#wget -O /tmp/Ciliora-Prima.zip http://cinnamon-spices.linuxmint.com/uploads/themes/F3YC-EWXY-XMK9.zip
	#unzip /tmp/Ciliora-Prima.zip -d /etc/skel/.themes
	#rm /tmp/Ciliora-Prima.zip
	# 修改二進制檔... 改以預設檔進行
